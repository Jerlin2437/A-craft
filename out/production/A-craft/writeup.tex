...
The efficiency difference between Backwards and Forwards A* on small grids is negligible. However Backwards A* will likely lead to suboptimal efficiency as the grid gets bigger and bigger. Since we are exploring nodes near the goal state first, we are essentially ignoring all the obstacles near the start state. In cases with large grids, oftentimes we will find the most quickest unblocked path from the goal state to the start area (area as in an undefined region where we start to see obstacles) but have to navigate around the entire grid to find a suitable entrance into the start area and into the start node. Forwards A star does the exact opposite, and will find an opening out of the start area first, before finding the quickest unblocked path from start area to goal state. 
